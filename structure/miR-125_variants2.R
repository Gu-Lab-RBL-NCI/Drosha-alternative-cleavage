...
mir16 <- "GCCAGUCUCUAGGUCCCUGAGACCCUUUAACCUGUGAGGACAUCCAGGGUCACAGGUGAGGUUCUUGGGAGCCUGGCGUCUGGC" #>hsa-mir-125a MI0000469
mir16 <- gsub("U", "T",mir16 )

#Recalculate Drosha processing score
variants_16 <- read.table(file="GSE67937_125_variant_counts.txt", header = TRUE, fill = FALSE, sep =  "\t")
...
cl_mir <- read.table(file="GSM1658992_125_barcode_cleavage_fragments.txt", header = TRUE, fill = FALSE, sep =  "\t")
cl_mir$counts <- 1
...
variant_count_mir <- read.table(file="GSM1658986_125_dictionary.txt", header = FALSE, fill = FALSE, sep =  "\t")
#barcount_mir <- read.table(file="GSM1658989_125_input_barcode_counts.txt", header = TRUE, fill = FALSE, sep =  "\t")
...
write.table(summary_fidelity, "mir125-variants-cleavage-distribution.txt", sep="\t", append = FALSE, col.names = TRUE, row.names = FALSE)
...
write.table(total_over100, "mir125-variants-energy-cleavage.txt", sep="\t", append = FALSE, col.names = TRUE, row.names = FALSE)
...
write.table(summary_energy, "mir125-summary-energy-cleavage.txt", sep="\t", append = FALSE, col.names = TRUE, row.names = FALSE)

reference <- total_over100[which(total_over100$variant==mir16),]



top1 <- total_over100[1:(nrow(total_over100)/100),]
top1 <- total_over100[1:(nrow(total_over100)/10),]
